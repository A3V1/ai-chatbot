...
def prepare_documents():
    ...
    for row in mysql_data:
        ...
        # Convert Decimal to float, datetime/date to isoformat string, fallback to str for other non-serializable types
...
def upload_vectorstore(index_name="insurance-chatbot", namespace="default"):
    ...
    # Optional: Clear existing data, but ignore if namespace does not exist
    try:
        index.delete(delete_all=True, namespace=namespace)
    except Exception as e:
        if "Namespace not found" in str(e):
            pass  # Safe to ignore if namespace does not exist
        else:
            raise
